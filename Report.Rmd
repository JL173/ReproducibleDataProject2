...
ggplot(dh_short, aes(x = Incidents,
  ...
  ggtitle("Total Harm caused by Weather Events",
...
colnames(state_f)[6] <- "values"
colnames(state_i)[7] <- "values"

# create a label to split data on plotting
state_n$label <- "Number of Events"
state_p$label <- "Property Damage $"
...
ggplot(state_harm, aes(x = values,
  ...
  labs(x = NULL,
       ...
       subtitle = "National Climatic Data Center Storm Events 1950 - 2011",
       caption = "*Property Damages are estimates\n1950-54 only tornado events recorded\n 1955-92 only tornado, thunderstorm wind and hail events keyed to digital\n1993-5 only tornado, thunderstorm wind and hail events keyed to digital") +
  ...
  scale_x_log10(breaks = c(1e1, 1e2, 1e3, 1e4, 1e5, 1e6),
